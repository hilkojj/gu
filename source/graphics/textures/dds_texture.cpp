...
SharedTexture DDSTexture::fromDDSFile(const char *path)
{
    DDSData data(path);
    ...
    return std::make_shared<Texture>(id, data.width, data.height);
}

DDSTexture::DDSData::DDSData(const char *path)
{
    FILE *fp;
    fp = fopen(path, "rb");
    if (!fp)
